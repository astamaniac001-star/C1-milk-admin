...
var ALLOWED_ORIGIN = process.env.ALLOWED_ORIGIN || "https://c1milk.netlify.app";
var buildCorsHeaders = (origin) => {
...
var jsonResponse = (statusCode, headers, payload) => ({
  statusCode,
  headers: { ...headers, "Content-Type": "application/json" },
  body: JSON.stringify(payload)
});
var errorResponse = (statusCode, headers, code, message) => jsonResponse(statusCode, headers, { success: false, error: { code, message } });
var getClientIP = (event) => event.headers["x-nf-client-connection-ip"] || (event.headers["x-forwarded-for"] || "").split(",")[0].trim() || "unknown";
var hashIP = (ip) => (0, import_crypto.createHash)("sha256").update(ip).digest("hex").slice(0, 16);
...
async function callAppsScript(body) {
  ...
  try {
    const response = await fetch(APPS_SCRIPT_URL, {
      ...
      redirect: "follow"
      // Let Node.js handle the 302 automatically!
    });
...
var handler = async function(event) {
  ...
  if (validation.statusCode) return validation;
