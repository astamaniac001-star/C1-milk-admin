...
export function useAppDerived({ customers, imports, bills, logs, brands, logDate, custSearch, custFilter, impFilter, billFilter }) {
  ...
  const filteredC = useMemo(() => filterCustomers(customers, custSearch, custFilter), [customers, custSearch, custFilter]);
  const filteredI = useMemo(() => filterImports(imports, impFilter), [imports, impFilter]);
  const filteredB = useMemo(() => filterBills(bills, billFilter), [bills, billFilter]);
  const activeBrandsCount = useMemo(() => brands.filter(b => b.status === "Active").length, [brands]);

  return {
    activeC, totalRevenue, pendingDues, confirmedStock, todayLogs,
    filteredC, filteredI, filteredB, activeBrandsCount,
  };
}
