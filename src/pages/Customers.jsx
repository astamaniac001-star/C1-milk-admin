...
export default function Customers({
  filtered, total, bills,
  search, onSearchChange,
  filter, onFilterChange,
  onOpenModal, onWhatsapp, onDeactivate,
}) {
  return (
    <div>
      <Section title="Customers" action={<Btn small onClick={() => onOpenModal("addCustomer")}>+ Add</Btn>} />
      <div style={{ display:"flex", gap:6, marginBottom:10 }}>
        <input value={search} onChange={e => onSearchChange(e.target.value)} placeholder="Search…" style={{ ...IS(), flex:1 }} />
        <select value={filter} onChange={e => onFilterChange(e.target.value)} style={{ ...IS(), width:90 }}>
          {STATUS_FILTERS.map(s => <option key={s}>{s}</option>)}
        </select>
      </div>
      <div style={{ fontSize:12, color:"#6b7280", marginBottom:8 }}>{total} customers</div>
      {filtered.length === 0 ? <Empty msg="No customers found" /> : filtered.map(c => (
        <Card key={c.id}>
          ...
          <div style={{ display:"flex", gap:6, marginTop:10, flexWrap:"wrap" }}>
            <Btn small variant="secondary" onClick={() => onOpenModal("editCustomer", c)}>Edit</Btn>
            {c.status === "Active" && <Btn small variant="secondary" onClick={() => onOpenModal("addPause", { custId: c.id })}>Pause</Btn>}
            <Btn
            ...
            >WhatsApp</Btn>
            <Btn small variant="danger" onClick={() => onDeactivate(c.id)}>Deactivate</Btn>
          </div>
        </Card>
      ))}
    </div>
