...
import { Card, Btn, IS, Section, StatGrid, Empty, Badge, ActiveBrandOptions } from "../components/ui.jsx";

const STATUS_FILTERS = ["Draft","Confirmed","Reconciled"];

export default function Imports({
  filtered, brands, impFilter, onImpFilterChange,
  onOpenModal, onConfirm, onDelete,
}) {
  const totalQty  = filtered.filter(i => i.status === "Confirmed").reduce((s, i) => s + i.qty, 0);
  const totalCost = filtered.filter(i => i.status === "Confirmed").reduce((s, i) => s + i.total, 0);
    ...
    <div>
      <Section title="Milk Imports" action={
        <div style={{ display:"flex", gap:6 }}>
          <Btn small variant="secondary" onClick={() => onOpenModal("addBrand")}>+ Brand</Btn>
          <Btn small onClick={() => onOpenModal("addImport")}>+ Import</Btn>
        </div>
      } />
      <div style={{ display:"flex", gap:6, marginBottom:10, flexWrap:"wrap" }}>
        <input type="month" value={impFilter.month}  onChange={e => onImpFilterChange({ ...impFilter, month:  e.target.value })} style={{ ...IS(), flex:1 }} />
        <select          value={impFilter.brand}  onChange={e => onImpFilterChange({ ...impFilter, brand:  e.target.value })} style={{ ...IS(), flex:1 }}>
          <option value="">All Brands</option>
          <ActiveBrandOptions brands={brands} />
        </select>
        <select          value={impFilter.status} onChange={e => onImpFilterChange({ ...impFilter, status: e.target.value })} style={{ ...IS(), flex:1 }}>
      ...
      {filtered.length === 0 ? <Empty msg="No imports match filters" /> : filtered.map(imp => (
        <Card key={imp.id}>
          ...
          {imp.status === "Draft" && (
            <div style={{ display:"flex", gap:6, marginTop:10 }}>
              <Btn small variant="secondary" onClick={() => onOpenModal("addImport", imp)}>Edit</Btn>
              <Btn small variant="success"   onClick={() => onConfirm(imp.id)}>Confirm</Btn>
              <Btn small variant="danger"    onClick={() => onDelete(imp.id)}>Delete</Btn>
            </div>
          )}
        </Card>
      ))}
