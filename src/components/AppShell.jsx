...
function TabBadge({ show }) {
  ...
  return (
    <span style={{
      position: "absolute", top: 6, right: "20%", width: 7, height: 7,
      background: "#ef4444", borderRadius: "50%", display: "block",
    }} />
  );
}
...
function TabButton({ tab, active, deadCount, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <span style={getTabLabelStyle(isActive)}>
        {tab.label}
      </span>
...
export function AppShell({ tab, today, queue, onTabChange, children, footer }) {
  ...
  return (
    <div style={{
      fontFamily: "system-ui,sans-serif", maxWidth: 420, margin: "0 auto",
      background: "#f8fafc", minHeight: 640, position: "relative", paddingBottom: 68,
    }}>
      <div style={{
        background: BLUE, color: "#fff", padding: "14px 16px 10px",
      ...
      <div style={{
        ...
        width: "100%", maxWidth: 420, background: "#fff", borderTop: "0.5px solid #e5e7eb",
        display: "flex", zIndex: 300,
      }}>
        {TABS.map(t => (
          <TabButton key={t.id} tab={t} active={tab} deadCount={deadCount} onSelect={onTabChange} />
        ))}
      </div>
